...
int size(Person *ptr){
	//total number of names in the myList
	if (ptr == NULL) return 0;
	return 1 + size(ptr->next);

}

int chars(Person *ptr) {
	//total number of characters in all names in the list
	if ( ptr == NULL ) return 0;
	int count = strlen(ptr->name);
	return count + chars(ptr->next);
}
