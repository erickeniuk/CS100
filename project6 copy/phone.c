...
MultiLinkedList *add(MultiLinkedList *list, char *first, char *last, long num) {
	...
	Node *next1 = curr1->nextFirst;

	Node *previous1 = list->headLastName;
	Node *current1 = previous1->nextLast;
	Node *nextnext1 = current1->nextLast;

///first name list
printf("Enter while loop first name\n");
while (curr1 && (strcasecmp(curr1->first, first) < 0) && (strcasecmp(curr1->first, first) != 0)) {
	prev1 = curr1;
	...
	newOne->nextFirst = next1;}


///last name list
printf("Enter while loop last name\n");
while (current1 && (strcasecmp(current1->first, first) < 0) && (strcasecmp(current1->first, first) != 0)) {
	previous1 = current1;
	current1 = current1->nextLast; }

	if (strcasecmp(last, current1->last) < 0) {
			printf("running if");
